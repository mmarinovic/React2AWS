...
import { VALID_FARGATE_CPU_UNITS, FARGATE_CPU_MEMORY_MAP } from '@/lib/constants/aws';
...
export function generateFargate(resource: FargateResource, vpcName?: string): string {
  ...
  const actualCpuUnits = VALID_FARGATE_CPU_UNITS.reduce((prev, curr) =>
    Math.abs(curr - cpu) < Math.abs(prev - cpu) ? curr : prev
  );

  const validMemory = FARGATE_CPU_MEMORY_MAP[actualCpuUnits] || [512];
