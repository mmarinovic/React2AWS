...
export function generateLambda(resource: LambdaResource, vpcName?: string): string {
  ...
  const vpcSanitized = vpcName?.replace(/[^a-zA-Z0-9_]/g, '_');

  // Map runtime shortcuts to AWS Lambda runtime identifiers
  ...
  // Security group for Lambda (only when in VPC)
...
` : '';
  ...
  // VPC config block for Lambda function
...
` : '';
  ...
  // Additional depends_on for VPC resources
