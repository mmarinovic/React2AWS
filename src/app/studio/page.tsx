...
import { Eye, FileCode, AlertCircle, Home, Code, Menu, X, Book, FileText, FolderOpen } from 'lucide-react';
...
export default function StudioPage() {
  ...
  const [mobilePanel, setMobilePanel] = useState<'docs' | 'reference' | 'examples'>('examples');
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-900">
      {/* Title Bar */}
      <div className="flex items-center justify-between border-b border-slate-700 bg-slate-800 px-3 py-2 md:px-4">
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-slate-200 hidden sm:inline">Infrastructure Studio</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-500 hidden sm:inline">React2AWS</span>
          {/* Mobile menu button */}
          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="flex md:hidden items-center justify-center h-8 w-8 rounded text-slate-400 hover:bg-slate-700 hover:text-slate-200"
          >
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {/* Side Panel - Hidden on mobile, shown as overlay when menu open */}
        <div className="hidden md:block">
          <SidePanel
        ...
        {mobileMenuOpen && (
          <div className="absolute inset-0 z-50 md:hidden">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-64 bg-slate-800 shadow-xl flex flex-col">
              {/* Mobile Panel Tabs */}
              <div className="flex border-b border-slate-700">
                <button
                  onClick={() => setMobilePanel('docs')}
                  className={`flex-1 flex items-center justify-center gap-1.5 px-2 py-2.5 text-xs transition-colors ${
                    mobilePanel === 'docs'
                      ? 'border-b-2 border-orange-500 text-slate-200'
                      : 'text-slate-400'
                  }`}
                >
